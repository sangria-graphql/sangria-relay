...
object Mutation {
  ...
  def fieldWithClientMutationId[Ctx, Val, Res: MutationLike: ClassTag, Input: FromInput](
    ...
    val inputType = InputObjectType[Input](
      typeName + "Input",
    ...
    val outputType = ObjectType(
      ...
      outputFields :+
        Field(
          ClientMutationIdFieldName,
          OptionType(StringType),
          resolve =
            (ctx: Context[Ctx, Res]) => implicitly[MutationLike[Res]].clientMutationId(ctx.value))
    )
...
@implicitNotFound(
  "Type ${T} can't be used as a Mutation. Please consider defining implicit instance of sangria.relay.MutationLike for type ${T} or extending sangria.relay.Mutation trait.")

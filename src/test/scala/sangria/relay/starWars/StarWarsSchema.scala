...
object StarWarsSchema {
  ...
  val FactionType: ObjectType[ShipRepo, Faction] = ObjectType(
    ...
    fields[ShipRepo, Faction](
      ...
      Field(
        ...
        resolve = ctx =>
          Connection.connectionFromSeq(ctx.value.ships.map(ctx.ctx.getShip), ConnectionArgs(ctx))
      )
    )
  )
  ...
  val shipMutation = Mutation
    .fieldWithClientMutationId[ShipRepo, Unit, ShipMutationPayload, InputObjectType.DefaultInput](
      ...
      inputFields =
        List(InputField("shipName", StringType), InputField("factionId", GlobalIdTypeAlias)),
      outputFields = fields(

      ...
      "list all ships with `nodes` fields" in {
        ...
        Executor.execute(StarWarsSchema.schema, doc, userContext = new ShipRepo).await should be(
          Map(
            "data" -> Map(
              "nodes" -> Vector(
                Map("id" -> "U2hpcDox", "name" -> "X-Wing"),
                Map("id" -> "U2hpcDoz", "name" -> "A-Wing"),
                null,
                Map("id" -> "RmFjdGlvbjox"),
                Map("id" -> "U2hpcDox", "name" -> "X-Wing")))))
      }
          ...
          Map(
            "data" -> Map("node" ->
              Map(
                "id" -> "RmFjdGlvbjox",
                "name" -> "Alliance to Restore the Republic",
                "ships" -> Map("edges" -> Vector(
                  Map("node" -> Map("name" -> "X-Wing")),
                  Map("node" -> Map("name" -> "Y-Wing")),
                  Map("node" -> Map("name" -> "A-Wing")),
                  Map("node" -> Map("name" -> "Millenium Falcon")),
                  Map("node" -> Map("name" -> "Home One"))
                ))
              ))))
      }
        ...
        Executor.execute(StarWarsSchema.schema, doc, userContext = new ShipRepo).await should be(
          Map("data" -> Map("nodes" -> Vector(
            ...
            Map(
              ...
              "ships" -> Map("edges" -> Vector(
                Map("node" -> Map("name" -> "TIE Fighter")),
                Map("node" -> Map("name" -> "TIE Interceptor")),
                Map("node" -> Map("name" -> "Executor"))))
            )
          ))))
      }
    }

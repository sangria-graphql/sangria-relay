...
class ConnectionSpec extends AnyWordSpec with Matchers with AwaitSupport {
  ...
  val ConnectionDefinition(_, friendConnection) = Connection.definition[Repo, Connection, User](
    ...
    nodeType = UserType,
    edgeFields =
      fields(Field("friendshipTime", OptionType(StringType), resolve = _ => "Yesterday")),
    connectionFields = fields(Field("totalCount", OptionType(IntType), resolve = _.ctx.Users.size))
  )
